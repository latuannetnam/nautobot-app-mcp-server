...
| Req ID | Requirement | Status |
|---|---|---|
| REFA-01 | `view.py`: replace `asyncio.run()` with `async_to_sync(_call_starlette_handler)` | **Completed** (WAVE2-VIEW, 21e2f6d; test fix, 67cdef5) |
| REFA-02 | `view.py`: `async with session_manager.run():` before `handle_request()` | **Completed** (WAVE2-VIEW, 21e2f6d; test fix, 67cdef5) |
| REFA-03 | `view.py`: ASGI scope dict built from Django request (server, scheme, client, path, query_string, headers, method, http_version) | **Completed** (WAVE2-VIEW, 21e2f6d; test fix, 67cdef5) |
